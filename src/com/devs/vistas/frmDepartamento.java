...
public void popup(){
 ...
 private void cargarModeloTabla() {
        ...
        List<Departamentos> lista = depdao.listAll();//agregarlos registros a la lista
  ...
  public void eliminar(){
         if(depselect!=null){
            try {
                depdao.Delete(depselect);
                cargarModeloTabla();
 new rojerusan.RSNotifyFade("Proceso Completado", "El Registro fue Eliminado Correctamente: "+depselect.getDescripcion(), 6, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.INFORMATION).setVisible(true);
...
public void guardar(){
    Departamentos depss=new Departamentos();
        try {
            if(txtdescripcion.getText().length()==0){
               ...
               // DesktopNotify.showDesktopMessage("Registros Incompletos", "Debes Ingresar una Descripcion", DesktopNotify.ERROR);
                ...
                depss.setDescripcion(txtdescripcion.getText());
                depdao.Save(depss);
                cargarModeloTabla();
                limpiartextbox();
                new rojerusan.RSNotifyFade("Proceso Completo", "Registro Agregado exitosamente: "+depss.getDescripcion(), 6, RSNotifyFade.PositionNotify.TopRight, RSNotifyFade.TypeNotify.SUCCESS).setVisible(true);
 ...
 public void modificar() {
        if(depselect!=null){
            try {
                depselect.setDescripcion(txtdescripcion.getText());
                depdao.Update(depselect);
                cargarModeloTabla();
                new rojerusan.RSNotifyFade("Proceso Completado", "Registro Modificado Correctamente a: "+depselect.getDescripcion(), 6, RSNotifyFade.PositionNotify.BottomRight, RSNotifyFade.TypeNotify.INFORMATION).setVisible(true);
  ...
  public void buscar(){
        ...
        List<Departamentos> lista = depdao.listAllParameter(parametro);
 ...
 public void imprimireporte(){
        ...
        try {
            report = JasperCompileManager.compileReport(new File("").getAbsolutePath()
                    + "/src/reportes/reportedepartamentos.jrxml");
            print = JasperFillManager.fillReport(report, p, cnn);
            JasperViewer view = new JasperViewer(print, false);
            view.setTitle("Reporte General de Departamentos");
            ...
            new ListSelectionListener(){ // Instanciamos
                ...
                public void valueChanged (ListSelectionEvent event){ // evento de la tabla
                    if(!event.getValueIsAdjusting() && (tablabanco.getSelectedRow()>=0) ) {
                        ...
                        */     
                        depselect = (Departamentos) modeloTabla.getValueAt(filaSeleccionada,0); 

                        // LLenamos los textBoxs atraves del objeto ...
                        txtid.setText(depselect.getCodDepartamento().toString());
                        txtdescripcion.setText(depselect.getDescripcion());
